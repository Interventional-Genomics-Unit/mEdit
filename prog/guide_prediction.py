...
def guide_prediction(args, jobtag):
	...
	config_template_path = project_file_path("config", "medit_guide_pred.yaml")
	cluster_template_path = project_file_path("config", "medit_cluster.yaml")
	...
	for smk_setup_idx in range(len(allowed_rules)):
		try:
			...
			launch_shell_cmd(f"snakemake "
			                 f"--snakefile {project_file_path('snakemake', 'guide_prediction.smk')} "
			                 f"-j {ncores} "
			                 f"{smk_run_triggers} "
