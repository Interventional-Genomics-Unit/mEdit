...
def reformat_ref_and_alt(lines, offtarget_genome, genome_type):
    ...
    for line in lines:
        ...
        if ref:
            ...
            # current .bed heading
            # 1) id 2) sequence 3) match_chrm 4) match_position 5) match_strand
            # 6) match_distance 7) match_sequence 8) rna_bulges 9) dna_bulges 10) specificity
            # 11) alt_site_impact 12) chrom 13) start 14) stop 15) alt_var 16) alt_genome'

            variant = line_split[11:] #chrom 13) start 14) stop 15) alt_var 16) alt_genome'
...
def compile_mutliple_alignments(dist_lines, max_bulge):
    ...
    ot_dict = {}
    # 'id,sequence,match_chrm,match_position,match_strand,match_distance,match_sequence,rna_bulges,dna_bulges,specificity,alt_site_impact,alt_var,alt_genome,Distance'
    for line in dist_lines[1:]:
        ...
        for i in range(0, max_bulge + 1):
            if f"{prefix}{str(pos + i)}{strand}" in ot_dict.keys():
                ot_dict[f"{prefix}{str(pos + i)}{strand}"].append(line)
                not_found = False
                break
            elif f"{prefix}{str(pos - i)}{strand}" in ot_dict.keys():
                ot_dict[f"{prefix}{str(pos - i)}{strand}"].append(line)
                not_found = False
                break
            else:
                pass
        if not_found:
            ot_dict[f"{prefix}{str(pos)}{strand}"] = [line]
...
def de_dup(dist_lines,max_bulge):
    """
    Collapse overlapping alignments at the same site to the single best.

    Preference (best → worst):
      1. Lower edit distance                              — line[-1]
      2. Higher mismatch count (more bulges taken as mm)  — line[5]
      3. Lower placement score (mismatches closest from 3' PAM) — line[6]

    Appends an "Alt Alignment [0=No/1=Yes]" column: 1 if this site had
    multiple candidate alignments that had to be resolved, else 0.
    """

    def placement_score(match_seq):
        # Sum of positions of mismatches (lowercase) and bulges ('-') in the
        # aligned match sequence. Lower means shifted toward the 5' end,
        # farther from the 3' PAM, which is generally the preferred alignment.
        return sum(i for i, ch in enumerate(match_seq[::-1])if ch.islower() or ch == "-")
    ...
    for coord,lines in ot_dict.items():
        if len(lines) ==1:
            ...
            alt_alignment = '1'
            dist, mm,p_score = 100,0,0
            bestline = ''

            for line in lines:
                s, d, m = placement_score(line[6]),int(line[-1]),int(line[5])
                if d < dist:
                    bestline = line
                    dist, mm, p_score = d, m ,s
                elif d == dist and s >= p_score:
                    bestline = line
                    dist, mm, p_score = d, m, s
                else:
                    pass
...
def load_chr_fasta(chr_fasta_path, chrom):
    """Load a per-chromosomefasta  pickle. Return None (with a logged warning)"""
    try:
        with open(chr_fasta_path, 'rb') as pfile:
            return pickle.load(pfile)
    except FileNotFoundError:
        logging.warning(f"FASTA pickle not found for {chrom}: {chr_fasta_path} — "
                        f"base-edit annotations for sites on {chrom} will be 'na'")
    except (pickle.UnpicklingError, EOFError) as e:
        logging.warning(f"FASTA pickle unreadable for {chrom}: {chr_fasta_path} "
                        f"({type(e).__name__}: {e}) — base-edit annotations for "
                        f"sites on {chrom} will be 'na'")
    return None
...
def add_be_annotations(df,annote_path,guide_params,fasta_path):
    ...
    for chrom, coord, start, seq in zip( df['match_chrm'], coords, df['match_position'],df.match_sequence):
        if ch != chrom:
            chr_fasta_path = f"{fasta_path}/chr{str(chrom).replace('chr', '')}.pkl"
            fasta = load_chr_fasta(chr_fasta_path, chrom)
            ch = chrom

        f,g,n,c = 'intergenic','.','na','na'

        target_bases = seq.upper().replace("-", "")[be_win[0] - 1:be_win[1] + 1]
        n = target_bases.count(conversion[0])

        pos_in_transcript = Transcript.transcript(coord)

        if pos_in_transcript != 'intergenic':
            g = pos_in_transcript.tx_info()[2]
            f = pos_in_transcript.feature

            if not fasta:
                try:
                    tx = Transcript.transcript(coord)
                    _, c = add_be_outcomes(tx, fasta, coord, Feature,seq, conversion, be_win)
    ...
    df['Gene'] = Gene
    df['Feature'] = Feature
    df['N_Base_Editable'] = nbases
    df['Base_Change_Consequence'] = cons
