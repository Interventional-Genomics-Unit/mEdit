...
class DeepCas9(object):
    ...
    def __init__(self, filter_size, filter_num, node_1=80, node_2=60, l_rate=0.005):
        ...
        def create_new_conv_layer(input_data, num_input_channels, num_filters, filter_shape, pool_shape, name):
            ...
            strides = [1, 1, 2, 1]
            out_layer = tf1.nn.avg_pool(out_layer, ksize=ksize, strides=strides,
                                        padding='SAME')
            return out_layer
        ...
        with tf1.variable_scope('Fully_Connected_Layer1'):
            ...
            L_flatten_0 = tf1.reshape(L_pool_0, [-1, node_num_0])
            L_flatten_1 = tf1.reshape(L_pool_1, [-1, node_num_1])
            L_flatten_2 = tf1.reshape(L_pool_2, [-1, node_num_2])
            L_flatten = tf1.concat([L_flatten_0, L_flatten_1, L_flatten_2], 1, name='concat')
            ...
            L_fcl1_pre = tf1.nn.bias_add(tf1.matmul(L_flatten, W_fcl1), B_fcl1)
            L_fcl1 = tf1.nn.relu(L_fcl1_pre)
            L_fcl1_drop = tf1.layers.dropout(L_fcl1, 0.3, self.is_training)
        ...
        with tf1.variable_scope('Fully_Connected_Layer2'):
            ...
            L_fcl2_pre = tf1.nn.bias_add(tf1.matmul(L_fcl1_drop, W_fcl2), B_fcl2)
            L_fcl2 = tf1.nn.relu(L_fcl2_pre)
            L_fcl2_drop = tf1.layers.dropout(L_fcl2, 0.3, self.is_training)
        ...
        with tf1.variable_scope('Output_Layer'):
            ...
            self.outputs = tf1.nn.bias_add(tf1.matmul(L_fcl2_drop, W_out), B_out)

        # Define loss function and optimizer
        self.obj_loss = tf1.reduce_mean(tf1.square(self.targets - self.outputs))
